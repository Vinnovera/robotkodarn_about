...
const { convertToAnchor, modifiers, linkResolver, serialize, icon } = require('../utils')
...
function section (state = {}, emit, slice, page, type = 'text') {
  ...
  const textblock = () => {
    ...
    return html`
      <div>
        ...
        ${hasWindow && hasMore ? expander.render(isExpanded, toggleSection, state.color) : null}
        ${hasMore && (isExpanded || !hasWindow) ? html`
          <div class="Section-content Section-content--appear">
            ...
            ${hasWindow && hasMore ? html`
              <button class="Button Button--toggle" onclick=${toggleSection}>
                <span class="Section-icon Section-icon--${state.color}">${icon('implode')}</span>
                ${__('Show less')}
              </button>
            ` : null}
          </div>
  ...
  return html`
    <article class=${modifiers('Section', { [type]: true, [page]: true })}>
      <h2 class="Section-title" id=${anchor}>${slice.primary.title}</h2>
