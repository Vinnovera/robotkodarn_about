    ...
    case 'article':
      return html`
        <div class="Hero-content">
          ...
          <div class="Hero-buttons">${renderAnchors(doc.data.body, opts.color)}</div>
        </div>
      `
    case 'error':
      return html`
        <div class="Hero-content">
          <header><h1 class="u-textXL u-marginBottomS">${doc.title}</h1></header>
          <p class="Hero-introduction u-marginAuto">${doc.description}</p>
        </div>
      `
    case 'thanks':
      return html`
...
const renderAnchors = (body, color = 'celestial') => {
  return body.map(slice => {
    ...
    if (!slice.primary.title) { return }

    const anchor = slice.slice_type === 'text' || slice.slice_type === 'team'

    if (anchor) {
      return html`
        <a href="#${convertToAnchor(slice.primary.title)}" onclick=${scrollToAnchor} class="${modifiers('Anchor', { [color]: true })}">
          ${slice.primary.title}
        </a>
      `
    }
...
function loading (opts) {
  const buttonModifiers = { article: true, sm: true, loading: true }
  const button = () => html`
      ...
      <div class="u-widthL">
        ...
        ${opts.pageType === 'article' || opts.pageType === 'contact'
          ? html`
            <div>
              <p>
                <span class="${modifiers('Hero-introduction', { loading: true })}">${__('Loading text')}</span>
              </p>
              ${button()}
              ${button()}
              ${button()}
