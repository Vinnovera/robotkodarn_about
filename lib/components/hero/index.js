  ...
  return html`
    <div class="u-flexWrapper">
      <section class="${modifiers('Hero', { [opts.pageType]: true })}">
        ${content}
        ${pattern(opts.color, opts.pageType)}
      </section>
    </div>
  `
}
...
const renderContent = (doc, opts, isLoading = false) => {
  const contentStyle = modifiers('Hero-content', { [opts.pageType]: true })
...
const introductionLoading = (type = '') => {
  return html`
  <p class="Hero-introduction Hero-introduction--loading u-loadingTransparent">
    ${__('Loading text')}
  </p>`
}
