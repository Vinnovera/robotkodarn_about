...
function navigation (state, emit, type, children) {
  ...
  // Use color that matches main color of the view (can be pink or blue)
  const color = type === 'article' ? 'blue' : 'pink'
  const style = `Navigation Navigation--${color}`
  const linkStyle = `Navigation-listLink Navigation-listLink--${color}`
    ...
    ${doc.data.navigation.map(item => {
      return html`
        <li class=${setClass('item')}>
          <a class=${linkStyle} href="/${item.link.uid ? item.link.uid : ''}">
            ${item.title}
          </a>
        </li>`
    })}
