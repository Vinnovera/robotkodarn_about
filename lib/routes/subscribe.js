...
router.post('/subscribe', async (ctx, next) => {
  try {
    ...
    // Exit if email does not exist
    if (!email) {
      throw new Error('No email address provided')
    }

    // got provides a nicer interface than the built-in http module
    ...
    if (!ctx.accepts('html')) {
      // Mailchimp returns error on err.response.body
      ctx.body = {
        status: err.response.body.title
      }

      ctx.status = err.response.body.status || 400
      ctx.type = 'application/json'
    } else {
      ctx.throw(400, err.body.title)
    }
  }
})
