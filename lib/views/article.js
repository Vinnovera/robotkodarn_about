    ...
    <div class="App App--left">
      ...
      ${doc.data.body.map(slice => {
        switch (slice.slice_type) {
          case 'text': return textblock(slice.primary, expand, state.expandedSections, opts.theme.color)
          case 'team': return team(slice, expand, state.expandedSections, opts.theme.color)
          default: return null
        }
      })}
    </div>
...
const setColors = (route) => {
  switch (route) {
    case 'sa-funkar-det': return { color: 'grass', dark: true }
    case 'om-robotkodarn': return { color: 'mauve', dark: true }
